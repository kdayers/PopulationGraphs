...
G = graph(A);          % Create a MATLAB graph object
D = distances(G);      % Compute all-pairs shortest path distances
...
COpen = mean(CLocOpen);
%disp('Closed Clustering Coefficient:')
%disp(CClosed)
%disp('Open Clustering Coefficient: ')
%disp(COpen)
...
for i = 1:n
    ...
    DSGClosed = distances(graph(MClosed{i}));
    DSGOpen = distances(graph(MOpen{i}));
